...
PALEO_ENV_FILE="$ENV_FILE_ABS" "${COMPOSE_CMD[@]}" --env-file "$ENV_FILE_ABS" -f "$COMPOSE_FILE" up -d --build
PALEO_ENV_FILE="$ENV_FILE_ABS" "${COMPOSE_CMD[@]}" --env-file "$ENV_FILE_ABS" -f "$COMPOSE_FILE" ps
