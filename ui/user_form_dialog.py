...
class UserFormDialog(tk.Toplevel):
    def __init__(self, parent: tk.Widget, initial_data: dict[str, str] | None, on_save):
        ...
        ttk.Label(frame, text="active").grid(row=2, column=0, sticky="e", padx=4, pady=4)
        self.active_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(frame, variable=self.active_var).grid(row=2, column=1, sticky="w", padx=4, pady=4)
