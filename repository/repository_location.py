    ...
    def ensure_locations_table(self) -> None:
        with self._connect() as conn:
            ...
            conn.execute(
                ...
                FROM "Finds" f
                WHERE NOT EXISTS (
                    SELECT 1 FROM "FindFieldObservations" fo WHERE fo.find_id = f.id
                )
                """
            )
            conn.execute(
                ...
                FROM "Finds" f
                WHERE NOT EXISTS (
                    SELECT 1 FROM "FindTaxonomy" ft WHERE ft.find_id = f.id
                )
                """
            )
